...
class Connection extends EventEmitter {
  ...
  handshake(appName, login, password, callback) {
    const packet = login && password ? this.createPacket('handshake', appName,
      'login', [login, password]) : this.createPacket('handshake', appName);
  ...
  //
  createPacket(kind, target, verb, args) {
    const packet = {
      [kind]: (
  ...
  _handshakeError(error) {
    ...
    const packet = this.createPacket('handshake', null, 'error',
      normalizedError);
