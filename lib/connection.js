...
class Connection extends EventEmitter {
  ...
  _end(message) {
    ...
    if (message) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        this._log('outgoingMessage', message);
      }
    } else {
      this.transport.end();
    }
  ...
  _processMessage(message) {
    ...
    if (keys.length === 0) {
      // heartbeat message
      if (process.env.NODE_ENV !== 'production') {
        this._log('heartbeat', message);
      }
      return;
    }

    if (process.env.NODE_ENV !== 'production') {
      this._log('incomingMessage', message);
    }
  ...
  _rejectMessage(message, fatal) {
    this._log('messageRejected', message);
  ...
  _processHandshakeRequest(message, keys) {
    ...
    this.server.emit(
      'log', this, 'handshakeRequest', applicationName, authStrategy
    );
  }
  ...
  _processHandshakeResponse(message) {
    ...
    this._log('handshake', message.error, message.ok);
  ...
  _processCallMessage(message, keys) {
    ...
    this._log('call', interfaceName, methodName, args);
  ...
  _processCallbackMessage(message) {
    ...
    this._log('callback', message.error, message.ok);
  ...
  _processEventMessage(message, keys) {
    ...
    this._log('event', interfaceName, eventName, eventArgs);
  ...
  _processInspectMessage(message) {
    ...
    this._log('inspect', interfaceName);
    this.session._onMessageReceived(messageId);
