...
function RemoteError(code, message) {
  ...
  this.message =
    message || RemoteError.defaultMessages[code] || code.toString();
...
RemoteError.prototype.toJstpArray = function() {
  const isMessagePresent =
    this.message && this.message !== this.code.toString();
  const isMessageStandard = RemoteError.defaultMessages.hasOwnProperty(
    this.code
  );
