// Utilities used by cli
'use strict';

const utils = {};
module.exports = utils;

utils.bindAll = (thisArg, source, target) => {
  Object.keys(source).map((field) => {
    target[field] = source[field].bind(thisArg);
    utils.bindAll(thisArg, source[field], target[field]);
  });
};

utils.filterFields = (obj, forbidden) => {
  const filtered = {};
  Object.keys(obj).forEach((field) => {
    filtered[field] = Object.keys(obj[field])
      .filter(key => forbidden.every(el => !key.startsWith(el)));
  });
  return filtered;
};
